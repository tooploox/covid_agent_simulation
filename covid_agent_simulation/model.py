...
class CoronavirusModel(Model):
    ...
    def setup_interior(self, row, column, id, interior_type, color="white", shape=None):
        interior = InteriorAgent(id, model=self, color=color,
                                 shape=shape, interior_type=interior_type)
            ...
            for c in range(grid_map.shape[1]):
                if grid_map[r, c] == 0:
                    self.setup_interior(r, c, grid_map[r, c], InteriorType.PARK,
                                        shape=self.config["patch"]["park"]["shape"],
                                        color=self.config["patch"]["park"]["color"])
                else:
                    self.setup_interior(r, c, grid_map[r, c], InteriorType.HOME,
                                        shape=self.config["patch"]["home"]["shape"],
                                        color=self.config["patch"]["home"]["color"])
